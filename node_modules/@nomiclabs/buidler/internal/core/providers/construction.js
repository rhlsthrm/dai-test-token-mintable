...
function createProvider(networkName, networkConfig, solcVersion, paths) {
    ...
    if (networkName === constants_1.BUIDLEREVM_NETWORK_NAME) {
        ...
        provider = new BuidlerEVMProvider(buidlerNetConfig.hardfork, constants_1.BUIDLEREVM_NETWORK_NAME, buidlerNetConfig.chainId, buidlerNetConfig.chainId, buidlerNetConfig.blockGasLimit, buidlerNetConfig.throwOnTransactionFailures, buidlerNetConfig.throwOnCallFailures, buidlerNetConfig.accounts, solcVersion, paths);
    }
    else {
        const httpNetConfig = networkConfig;
        provider = new http_1.HttpProvider(httpNetConfig.url, networkName, undefined, httpNetConfig.timeout);
    }
